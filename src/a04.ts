...
validators.set("byr", createValidator(/^(\d{4})$/, groups => {
  const year = parseInt(groups[1]);
  return year >= 1920 && year <= 2002;
}));

validators.set("iyr", createValidator(/^(\d{4})$/, groups => {
  const year = parseInt(groups[1]);
  return year >= 2010 && year <= 2020;
}));

validators.set("eyr", createValidator(/^(\d{4})$/, groups => {
  const year = parseInt(groups[1]);
  return year >= 2020 && year <= 2030;
}));

validators.set("hgt", createValidator(/^(\d+)(cm|in)$/, groups => {
...
lines.forEach(line => {
  if (line === "") {
    ...
    line.split(" ").map(pair => pair.split(":")).forEach(([field, value]: string[]) => {
