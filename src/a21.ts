...
p(allIngredients.filter(ingredient => {
  return [...allergenPossibilities.entries()].find(([allergen, ingredients]) => ingredients.has(ingredient)) === undefined;
}).length);
...
p([...allergens.keys()].sort().map(allergen => allergens.get(allergen)).join(","));
