...
lines.forEach(line => {
  ...
  if (containSpecs !== "no other bags.") {
    rules.set(outerColor, containSpecs.split(", ").map(containSpec => {
...
function countTotalBags(color: string): number {
  return 1 + sum((rules.get(color) || []).map(quantityAndColor => quantityAndColor.quantity * countTotalBags(quantityAndColor.color)));
}
